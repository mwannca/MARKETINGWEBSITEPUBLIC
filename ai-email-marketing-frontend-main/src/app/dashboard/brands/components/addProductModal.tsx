...
export default function AddProductModal({ isShow, setShow, brandId, setFinishedScraping }: AddProductModal) {
    ...
    const [prodUrl, setProdUrl] = useState<string>(''); // Initialize as an empty string
    ...
    const handleSubmit = async () => {
        if (prodUrl) {
            ...
            if (response) {
                ...
                setProdUrl(''); // Clear the input field after successful submission
            } else {
    ...
    useEffect(() => {
        if (isShow) {
            setProdUrl(''); // Reset the product URL when the modal is opened
            setLoading(false); // Ensure loading state is false when modal opens
        }
    }, [isShow]);
                        ...
                        <div className='mt-4'>
                            <div>
                                ...
                                <Input
                                    ...
                                    value={prodUrl} // Keep this as a controlled input
                                    disabled={loading}
                                    placeholder='https://www.nike.com/in/t/pegasus-41-blueprint-road-running-shoes-9ln3lK/HF0013-900'
                                />
    ...
    return null; // Return null when modal is not shown
}
