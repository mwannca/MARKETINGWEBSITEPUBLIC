...
import { fetchSvgFromS3, updateBrands } from '../actions'
...
export default function Logos({ logos, brandId, brand }: Logos) {
    ...
    const [svgContent, setSvgContent] = useState<Record<number, string>>({}); // To store SVG content

    useEffect(() => {
        setArrayOfLogos(logos);
    }, [logos]);

    // UseEffect to fetch SVG content when necessary
    useEffect(() => {
        arrayOfLogos.forEach(async (logo, index) => {
            if (logo.endsWith('.svg') && !svgContent[index]) {
                const svg = await fetchSvgFromS3(logo);
                if (svg) {
                    setSvgContent((prev) => ({
                        ...prev,
                        [index]: svg
    ...
    }, [arrayOfLogos, svgContent]);

    const handleDelete = async (index: number) => {
        const array = [...arrayOfLogos];
        array.splice(index, 1);
        setArrayOfLogos(array);
        await updateBrands(brand.id, brand.brand_name, brand.colors, brand.fonts, array);
    }

    const renderLogo = (logo: string, index: number) => {
        if (logo.endsWith('.svg')) {
            // If the SVG content is not yet fetched, show loading
            if (!svgContent[index]) {
                return (
                    <div className='bg-zinc-700 border cursor-pointer hover:opacity-30 border-zinc-500 rounded-xl h-[200px] w-[200px]' key={index}>
                        Loading...
                    </div>
                );
            }

            // Once fetched, render the SVG content
            return (
                <div onClick={() => { setSelectedImage(index); setDeleteModalShow(true); }} className='bg-zinc-700 border cursor-pointer hover:opacity-30 border-zinc-500 rounded-xl h-[200px] w-[200px]' key={index}>
                    <div dangerouslySetInnerHTML={{ __html: svgContent[index] }} style={{ width: '200px', height: '200px' }} />
                </div>
            );
        }
        ...
        return (
            <div onClick={() => { setSelectedImage(index); setDeleteModalShow(true); }} className='bg-zinc-700 border cursor-pointer hover:opacity-30 border-zinc-500 rounded-xl h-[200px] w-[200px]' key={index}>
                <Image src={logo} alt={`logo ${index}`} width={200} height={200} unoptimized />
            </div>
        );
    };
    ...
    return (
        <div>
            ...
            <div className='grid grid-cols-5 mt-3 overflow-auto h-[400px]'>
                ...
                {arrayOfLogos.map((logo, index) => renderLogo(logo, index))}
            </div>
