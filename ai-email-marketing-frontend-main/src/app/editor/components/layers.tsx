...
const StyledDiv = styled.div<{ depth: number; selected: boolean }>`
  ...
  align-items: center;
  padding: 4px 10px;
  background: ${(props) => (props.selected ? '#3f3f46' : 'transparent')}; /* Zinc-700 */
  color: ${(props) => (props.selected ? '#fff' : 'inherit')};
  svg {
    fill: ${(props) => (props.selected ? '#fff' : '#a1a1aa')}; /* Zinc-400 */
    margin-top: 2px;
  }
  .inner {
    flex: 1;
...
const Hide = styled.a<{ selected: boolean; isHidden: boolean }>`
  ...
  &:after {
    ...
    left: 2px;
    top: 3px;
    background: ${(props) => (props.selected ? '#fff' : '#a1a1aa')}; /* Zinc-400 */
...
export const LayerHeader = () => {
  ...
  const layerHeaderRef = useRef<HTMLDivElement>(null);

  // Apply the drag connector to dragRef
  useEffect(() => {
    if (dragRef.current) {
  ...
  // Apply the layerHeader connector to layerHeaderRef
  useEffect(() => {
    if (layerHeaderRef.current) {
      layerHeader(layerHeaderRef.current);
    }
  }, [layerHeader]);

  return (
      <StyledDiv selected={selected} depth={depth} ref={dragRef}>
        <Hide
            selected={selected}
            isHidden={hidden}
