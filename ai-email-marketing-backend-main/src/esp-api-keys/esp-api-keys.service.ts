...
export class EspApiKeysService {
  ...
  async fetchKeys(userId: string, res: Response) {
    const keys = await this.prisma.eSP.findMany({
  ...
  async createkey(dto: CreateKeyDto, userId: string, res: Response) {
    ...
    const newKey = await this.prisma.eSP.create({
      data: {
        user_id: userId,
  ...
  async createTemplate(
    ...
    const apiKey = await this.prisma.eSP.findUnique({
      where: { id: apiKeyId },
    });
    if (apiKey.user_id !== userId)
      return new UnauthorizedException(
        'You are unauthorized to access this api key',
      );
    ...
    console.log(data);
    if (response.ok) {
      const template = await this.prisma.klaviyoTemplates.create({
        data: {
  ...
  async createCampaign(dto: CreateCampaignDto, user_id: string, res: Response) {
      ...
      body: JSON.stringify({
        data: {
          ...
          attributes: {
            ...
            send_strategy: {
              method: 'static',
              options_static: { datetime: datetime },
            },
            tracking_options: {
              utm_params: [{ name: 'utm_medium', value: 'campaign' }],
            },
            'campaign-messages': {
              data: [
                {
    ...
    if (createCampaignResponse.ok) {
      const campaign = await this.prisma.klaviyoCampaigns.create({
        data: {
          user_id: user_id,
          campaign_id: campaignData.data.id,
        },
      });
      const assignTemplateUrl =
      ...
      if (assignTemplateResponse.ok) {
        return res.send({
          ...
          assignTemplateData: assignTemplateData.data,
        });
      } else {
  ...
  encrypt(key: string) {
    ...
    const encryptedKey =
      cipher.update(key, 'utf8', 'hex') + cipher.final('hex');
  ...
  decrypt(key: string) {
    ...
    const decryptedKey =
      dicipher.update(key, 'hex', 'utf8') + dicipher.final('utf8');
