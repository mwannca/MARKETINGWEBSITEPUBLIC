...
export class EditorService {
  ...
  async manualEditingSession(
    userId: string,
    dto: ManualEditSessionDto,
    res: Response,
  ) {
    const { product_id, product_name, brand_id } = dto;

    const editSession = await this.prisma.editor.create({
      data: {
        user_id: userId,
  ...
  async fetchEditingSession(sessionId: string, res: Response) {
    const session = await this.prisma.editor.findUnique({
      where: { id: sessionId },
    });
  ...
  async fetchAllEditingSessions(userId: string, res: Response) {
    const sessions = await this.prisma.editor.findMany({
      where: { user_id: userId },
    });
    return res.send({ message: 'Success', sessions }).status(200);
  }

  async saveSession(dto: SaveSessionDto, req, res: Response) {
    ...
    // Get the userId from the authenticated request (via AuthGuard)
    const userId = req.user.sub;

    // Generate the preview URL by saving the HTML as an image in S3
    const previewUrl = await this.htmlToImageSave(html, session.id, userId); // Pass userId

    // Update the session in the database with the new preview image URL and email saves
    const updatedEditSession = await this.prisma.editor.update({
      where: { id: session_id },
      data: {
  ...
  async newSessionFromTemplate(
    dto: SessionFromTemplateDto,
    user_id: string,
    res: Response,
  ) {
    ...
    if (!editSession)
      return new BadRequestException('Failed to create session');
    return res.send({ message: 'Success', editSession }).status(200);
  }
  ...
  async htmlToImageSave(html: string, fileName: string, userId: string) {
    const browser = await puppeteer.launch();
    console.log('Browser Launched');
    const page = await browser.newPage();
    console.log('New Page');
    ...
    try {
      ...
      // Upload the file to S3 using the userId
      const url = await this.aws.uploadFileExport(
        userId, // Pass userId to ensure the file is stored in the correct user folder
        filePath,
        fileName,
        'image/png',
      );
      ...
      console.error('Error during HTML to Image save:', error);
  ...
  async getAssets(sessionId: string, res: Response) {
    const { assets } = await this.prisma.editor.findUnique({
      where: { id: sessionId },
    });
  ...
  async addAssets(dto: addImageToAssetsDto, res: Response) {
    ...
    const updatedSession = await this.prisma.editor.update({
      where: { id: sessionId },
      data: {
